...
socketio = SocketIO(
    ...
    logger=False,
    # ========== CRITICAL FIXES FOR SESSION ERRORS ==========
    manage_session=True,  # Enable session management
    cookie='session_id',   # Use custom cookie name
    always_connect=True,   # Always allow reconnection
...
active_users = {}  # Store active user sessions
call_lock = Lock()
    ...
    try:
        with call_lock:
            if caller_id in active_calls and active_calls[caller_id].get('state') == 'calling':
                ...
                socketio.emit('call_timeout', {'message': 'No answer'}, room=caller_id)
...
def handle_connect():
    ...
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated connection: {request.sid}")
        return False
    
    # Join user's personal room
    join_room(str(current_user.id))
    
    # Track active user with session ID
    active_users[current_user.id] = {
        'sid': request.sid,
        'username': current_user.username,
        'connected_at': get_utc_time()
    }
...
def handle_register_user(data):
    """Register user with their ID for targeted messaging"""
    if not current_user.is_authenticated:
    ...
    if user_id and user_id == current_user.id:
        ...
        logger.info(f"✅ User REGISTERED: {user_id} ({user_name})")
...
def handle_call_user(data):
    ...
    logger.info(f"📞 CALL_USER: {current_user.id} -> {target_id} ({call_type})")
...
def handle_answer_call(data):
    ...
    logger.info(f"📞 ANSWER_CALL: {current_user.id} answering {caller_id}")
...
def handle_reject_call(data):
    ...
    # Notify caller
    emit('call_rejected', {'message': 'Call rejected', 'rejecter_id': current_user.id}, room=caller_id)
...
def handle_ice_candidate(data):
    ...
    emit('ice_candidate', {'candidate': candidate, 'from_id': current_user.id}, room=target_id)
...
def handle_end_call(data):
    ...
    logger.info(f"📴 END_CALL: {current_user.id} ending call")
    
    # If target_id not provided, find it
    if not target_id:
        with call_lock:
            if current_user.id in active_calls:
                target_id = active_calls[current_user.id].get('with')
    ...
    # Notify other party
    if target_id:
        emit('call_ended', {'message': 'Call ended', 'ended_by': current_user.id}, room=target_id)
    
    emit('call_ended', {'message': 'Call ended'}, room=current_user.id)
...
def handle_call_heartbeat(data):
    ...
    user_id = data.get('user_id')
    if user_id and user_id == current_user.id:
        emit('call_heartbeat_ack', {'status': 'ok', 'timestamp': get_utc_time()}, room=user_id)
...
if __name__ == '__main__':
    ...
    # Use socketio.run with proper settings for production
    socketio.run(
        app, 
        host='0.0.0.0', 
        port=port, 
        ...
        use_reloader=False  # Disable reloader to prevent session issues
    )