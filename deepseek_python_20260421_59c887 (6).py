...
# SocketIO - optimized for production with manage_session=True
socketio = SocketIO(
    app, 
...
def handle_call_timeout(caller_id, target_id):
    ...
    try:
        logger.info(f"Call timeout: caller={caller_id}, target={target_id}")
        with call_lock:
...
@login_required
def logout():
    """Secure logout with POST method only"""
    user_id = current_user.id  # Store before logout_user()
    logger.info(f"Logout started for user: {user_id}")
    
    try:
        # Cancel status timers
        if user_id in status_update_timers:
            try:
                status_update_timers[user_id].cancel()
                del status_update_timers[user_id]
                logger.info(f"Cancelled status timers for user {user_id}")
            except Exception as e:
                logger.error(f"Error cancelling status timer: {e}")
        ...
        logger.info(f"Updated offline status for user {user_id}")
        ...
        # End active calls safely
        with call_lock:
            if user_id in active_calls:
                target_id = active_calls[user_id].get('with')
                logger.info(f"Ending active call between {user_id} and {target_id}")
        ...
        if user_id in call_timeouts:
            try:
                ...
                logger.info(f"Cancelled call timeout for user {user_id}")
            except Exception as e:
                logger.error(f"Error cancelling call timeout: {e}")
        
    except Exception as e:
        logger.error(f"Logout error during cleanup: {e}")
...
def logout_get_fallback():
    """GET logout fallback for simple redirects"""
    user_id = current_user.id
    logger.info(f"GET Logout fallback for user: {user_id}")
    
    # Same cleanup as POST logout
    try:
        if user_id in status_update_timers:
            try:
                status_update_timers[user_id].cancel()
        ...
        logger.error(f"GET Logout error: {e}")
...
def handle_connect():
    ...
    # CRITICAL: Join user's private room for targeted messaging
    user_room = str(current_user.id)
    join_room(user_room)
    logger.info(f"User {current_user.id} joined room {user_room}")
...
def handle_disconnect():
    ...
    if current_user.is_authenticated:
        ...
        logger.info(f"User {current_user.id} left room {user_room}")
...
def handle_call_user(data):
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated call attempt from {request.sid}")
    ...
    if not target_id or not call_type:
        logger.warning(f"Missing call parameters from {current_user.id}")
    ...
    with call_lock:
        if current_user.id in active_calls:
            logger.warning(f"User {current_user.id} already in a call")
            emit('call_error', {'message': 'You are already in a call'}, room=request.sid)
            return
        
        if target_id in active_calls:
            logger.warning(f"Target {target_id} is already in a call")
    ...
    try:
        ...
        if not target_user.data or not target_user.data[0].get('is_online', False):
            logger.warning(f"Target {target_id} is offline")
    ...
    with call_lock:
        ...
        logger.info(f"Call initiated: {current_user.id} -> {target_id} ({call_type})")
    ...
    logger.info(f"Incoming call sent to room {target_id}")

@socketio.on('answer_call')
def handle_answer_call(data):
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated answer attempt from {request.sid}")
        emit('call_error', {'message': 'Not authenticated'}, room=request.sid)
        return
    ...
    if not caller_id or not answer_sdp:
        logger.warning(f"Missing answer parameters from {current_user.id}")
        emit('call_error', {'message': 'Missing parameters'}, room=request.sid)
        return
    ...
    with call_lock:
        if caller_id not in active_calls:
            logger.warning(f"Call from {caller_id} no longer exists")
        ...
        if caller_id in active_calls:
            ...
            logger.info(f"Call connected: {caller_id} <-> {current_user.id}")
    ...
    logger.info(f"Call answer sent to room {caller_id}")
...
def handle_reject_call(data):
    ...
    caller_id = data.get('caller_id')
    logger.info(f"Call rejected: {current_user.id} rejected call from {caller_id}")
    ...
    socketio.emit('call_rejected', room=str(caller_id))
...
def handle_ice_candidate(data):
    ...
    if target_id and candidate:
        logger.info(f"ICE candidate exchanged: {current_user.id} -> {target_id}")
...
def handle_end_call(data):
    ...
    logger.info(f"Call ended: {current_user.id} with {target_id}")
    
    # Clean up call state for both users
    with call_lock:
        if current_user.id in active_calls:
            del active_calls[current_user.id]
        if target_id and target_id in active_calls:
