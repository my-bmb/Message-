...
def save_live_message(sender_id, sender_name, content, msg_type='text', file_name=None, file_size=None, duration=None, reply_to_id=None, reply_to_content=None):
    ...
    try:
        ...
        if result.data:
            ...
            # Update cache
            update_live_chat_cache(saved_msg)
            return saved_msg
    except Exception as e:
        logger.error(f"Save live message to DB error: {e}")
        # Try to create table if it doesn't exist
        try:
            # Fallback: try to insert again after potential table creation
            result = supabase.table('live_chat_messages').insert(data).execute()
            if result.data:
                saved_msg = result.data[0]
                update_live_chat_cache(saved_msg)
...
def update_live_chat_cache(message):
    """Update in-memory cache with latest message"""
...
def create_live_chat_table():
    """Create live_chat_messages table if it doesn't exist"""
    try:
        ...
        # because Supabase Python client doesn't support raw DDL directly
        logger.info("Please create live_chat_messages table in Supabase using the SQL provided")
        return True
    except Exception as e:
...
if __name__ == '__main__':
    ...
    # Try to ensure database table exists
    try:
        create_live_chat_table()
    except:
        pass
