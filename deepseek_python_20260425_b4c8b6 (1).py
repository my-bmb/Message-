...
def get_nearby_users(user_id, lat=None, lng=None, radius_km=None, limit=50):
    ...
    try:
        ...
        if lat is not None and lng is not None:
            ...
            # No location provided - show all users sorted by online status
            for user in users:
                user['distance_display'] = "Unknown"
                user['distance_km'] = 999999
                if user.get('last_seen'):
...
def users():
    ...
    if lat is not None and lng is not None:
        ...
        # ✅ FILTER REMOVED - radius_km=None means no distance filtering, just calculation
