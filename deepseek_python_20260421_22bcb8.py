...
app.config['SESSION_COOKIE_SECURE'] = True if os.environ.get('RENDER', False) else False
...
# SocketIO with gevent and session support
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='gevent', manage_session=True, ping_timeout=60, ping_interval=25)
...
# Socket.IO authentication decorator
def socketio_login_required(f):
    @functools.wraps(f)
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated:
            emit('call_error', {'message': 'Authentication required'}, room=request.sid)
            return False
...
def get_messages_between(u1, u2, limit=20, offset=0):
    try:
        ...
        print(f"get_messages_between error: {e}")
        return []
...
def get_reactions_for_messages(message_ids):
    ...
    try:
        ...
        print(f"get_reactions_for_messages error: {e}")
        return {}
...
def remove_reaction(message_id, user_id, reaction):
    try:
        ...
        print(f"remove_reaction error: {e}")
        return False
...
def get_reactions_for_message(message_id):
    try:
        ...
        print(f"get_reactions_for_message error: {e}")
        return []
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_data and check_password_hash(user_data[0]['password_hash'], password):
            ...
            try:
                supabase.table('chat_users').update({'is_online': True, 'last_seen': get_utc_time()}).eq('id', user.id).execute()
...
def chat(user_id):
    try:
        ...
        print(f"Chat user fetch error: {e}")
...
@socketio.on('connect')
def handle_connect():
    """Handle socket connection - Fixed for AnonymousUserMixin"""
    if current_user.is_authenticated:
        join_room(str(current_user.id))
        try:
            supabase.table('chat_users').update({'is_online': True, 'last_seen': get_utc_time()}).eq('id', current_user.id).execute()
        ...
        print("⚠️ Anonymous user tried to connect via Socket.IO - allowing connection but no room join")
...
def handle_send_message(data):
    ...
    if reply_to_id:
        try:
            ...
            print(f"Fetch reply content error: {e}")
...
# WebRTC signaling events - FIXED with authentication
