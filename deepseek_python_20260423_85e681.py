...
def parse_location(location_str):
    """Parse POINT(lng lat) format to (lat, lng) tuple"""
...
def get_nearby_users(user_id, lat=None, lng=None, radius_km=50, limit=50):
    ...
    try:
        ...
        if lat is not None and lng is not None:
            for user in users:
                ...
                if user_lat is not None and user_lng is not None:
                    ...
                    user['distance_km'] = 999999  # Far away for sorting
            ...
            # Filter by radius if specified and not unknown distance
            if radius_km and radius_km < 999:
                ...
                for user in users:
                    if user.get('distance_km', 999999) <= radius_km:
                        ...
                        # Keep users with unknown distance but put them at the end
                        filtered_users.append(user)
                users = filtered_users
        
        else:
            # No location provided, just return all users without distance
            for user in users:
                user['distance_display'] = "Unknown"
        ...
        # Final fallback: return all users without distance
        try:
            result = supabase.table('chat_users')\
                .select('*')\
                .neq('id', user_id)\
...
def update_user_status(user_id, is_online):
    ...
    try:
        ...
        }, to=None)  # Fixed: broadcast replaced with to=None
    except Exception as e:
...
def update_location():
    ...
    try:
        # Store location as WKT format (POINT(lng lat))
        location_wkt = f"POINT({lng} {lat})"
        
        # Update using normal supabase client
        supabase.table('chat_users').update({
        ...
        # Emit to all connected clients using to=None (broadcast)
...
def handle_location_update(data):
    ...
    try:
        ...
        # Broadcast to others (use to=None for broadcast, include_self=False)
...
def handle_call_user(data):
    ...
    # Send incoming call with network status
    emit('incoming_call', {
...
def handle_answer_call(data):
    """Handle call answer with TURN status forwarding"""
    if not current_user.is_authenticated:
        emit('call_error', {'message': 'Not authenticated'}, room=request.sid)
        return
