...
# ----------------- SOCKETIO EVENTS (ALL ORIGINAL + NEW ONES) -----------------
@socketio.on('connect')
def handle_connect():
...
# ==================== NEW WEBRTC SIGNALING EVENTS WITH FIXES ====================
...
def handle_check_online(data):
    ...
    try:
        result = supabase.table('chat_users').select('is_online').eq('id', target_id).execute()
        if result.data:
...
def handle_answer_call(data):
    ...
    with call_lock:
        if caller_id not in active_calls:
            logger.warning(f"Late answer from {current_user.id} for caller {caller_id}, re-creating call state")
    ...
    # Send answer with network status
    emit('call_answered', {
        'answer': answer_sdp,
