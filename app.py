...
call_lock = Lock()
ping_timers = {}  # For signaling ping tracking
...
def format_distance(meters):
    """Format distance in meters to human readable format (Meters/KM)"""
    if meters is None:
        return "Unknown"
...
# ========== NEARBY USERS FUNCTION (UPDATED) ==========
def get_nearby_users(user_id, lat=None, lng=None, radius_km=50, limit=50):
    """
    Get users sorted by distance (nearest first)
    If lat/lng not provided, falls back to normal all users
    Returns users with distance_display field
    """
    try:
        if lat is not None and lng is not None:
            # Query using PostGIS to calculate distance in meters
            query = f"""
                SELECT 
                    id, 
                    username, 
                    ...
                    ST_Distance(
                        location,
                        ST_SetSRID(ST_MakePoint({float(lng)}, {float(lat)}), 4326)::geography
                    ) AS distance_meters
                FROM chat_users
                WHERE id != '{user_id}'
                    AND location IS NOT NULL
                ORDER BY distance_meters ASC, is_online DESC
                LIMIT {limit}
            """
            
            # Try to execute raw SQL
            try:
                result = supabase.rpc('pg_execute_sql', {'query': query}).execute()
                users = result.data if result.data else []
            except Exception as rpc_error:
                logger.warning(f"RPC method failed, trying alternative: {rpc_error}")
                # Alternative: Use supabase client with PostGIS functions
            ...
            # Add formatted distance display
            for user in users:
                meters = user.get('distance_meters')
                if meters is not None:
                    user['distance_display'] = format_distance(meters)
                    user['distance_km'] = round(float(meters) / 1000, 2) if meters else None
                else:
            ...
            # If no users with location found, fallback to all users
            if not users:
                logger.info(f"No nearby users with location found for {user_id}, falling back to all users")
                result = supabase.table('chat_users')\
                    .select('*')\
                    .neq('id', user_id)\
                    .execute()
            ...
            # No location provided, return all users
            result = supabase.table('chat_users')\
                .select('*')\
                .neq('id', user_id)\
                .execute()
        ...
        # Final fallback: return all users without location sorting
        try:
            result = supabase.table('chat_users')\
                .select('*')\
                .neq('id', user_id)\
...
@login_required
def users():
    """Display users list sorted by distance (nearest first)"""
    # Get location from query params (frontend sends via ?lat=&lng=)
    lat = request.args.get('lat', type=float)
    lng = request.args.get('lng', type=float)
    
    # Also check session (in case stored from previous)
    ...
    # Get users based on location availability
    if lat is not None and lng is not None:
        ...
        all_users = get_nearby_users(current_user.id)  # fallback to all users
...
def update_location():
    ...
    try:
        ...
        sql = f"""
            UPDATE chat_users 
            SET location = ST_SetSRID(ST_MakePoint({float(lng)}, {float(lat)}), 4326)::geography
            WHERE id = '{current_user.id}'
        """
        
        try:
            # Try RPC method
            supabase.rpc('pg_execute_sql', {'query': sql}).execute()
        except Exception as rpc_error:
            logger.warning(f"RPC update failed, trying alternative: {rpc_error}")
            # Alternative: Try to update without PostGIS (just store as text)
            # This is a fallback - better to have PostGIS enabled
            location_text = f"POINT({lng} {lat})"
            supabase.table('chat_users').update({'location': location_text}).eq('id', current_user.id).execute()
...
# ==================== LOCATION & NEARBY USERS SOCKET EVENTS ====================

@socketio.on('location_update')
def handle_location_update(data):
    """Real-time location update from frontend"""
    if not current_user.is_authenticated:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"RPC location update failed: {rpc_error}")
        ...
        # Get nearby users for the current user
        nearby_users = get_nearby_users(current_user.id, lat, lng, radius_km=10, limit=30)
        
        # Send updated list back to current user
        ...
        # Broadcast to others that this user's location changed (optional)
...
def handle_network_status(data):
    """Receive and forward network status to peer (TURN/STUN detection)"""
    if not current_user.is_authenticated:
        return
...
def handle_check_online(data):
    """Check if user is online and receiving messages"""
    if not current_user.is_authenticated:
        return
...
def handle_ping_receiver(data):
    """Ping receiver to check if they can receive calls"""
    if not current_user.is_authenticated:
        return
...
def handle_call_pong(data):
    """Response to ping - receiver is ready"""
    if not current_user.is_authenticated:
        return
...
def handle_call_user(data):
    """Handle outgoing call with TURN status forwarding"""
    if not current_user.is_authenticated:
    ...
    # Set timeout for answer (45 seconds for better reliability)
    ...
    # Send incoming call with network status (caller's status)
    emit('incoming_call', {
...
def handle_answer_call(data):
    """Handle call answer with TURN status forwarding and late answer recovery"""
    if not current_user.is_authenticated:
        emit('call_error', {'message': 'Not authenticated'}, room=request.sid)
        return
    ...
    # Remove timeout timer if exists
    if caller_id in call_timeouts:
        try:
    ...
    # Handle late answer - if call state was removed, re-create it
    with call_lock:
    ...
    # Send answer with network status (answerer's status)
    emit('call_answered', {
        'answer': answer_sdp,
        'answerer_needs_turn': answerer_needs_turn
    }, room=caller_id)
