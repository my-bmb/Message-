...
# Configure session for Supabase with retries
...
# SocketIO - FIXED: manage_session=True, removed auto-reconnect settings
socketio = SocketIO(
    app,
...
# ========== CENTRALIZED CALL END FUNCTION ==========
def end_call(user_id):
    """Centralized function to end a call safely - prevents duplicate events"""
    with call_lock:
        if user_id not in active_calls:
            print(f"⚠️ Call end skipped: {user_id} not in active_calls")
            return  # already ended
        
        target_id = active_calls[user_id].get("with")
        
        # Prevent duplicate execution
        if active_calls[user_id].get("state") == "ended":
            print(f"⚠️ Call end skipped: {user_id} call already ended")
            return
        
        print(f"✅ Ending call: {user_id} with {target_id}")
        
        # Mark both as ended
        active_calls[user_id]["state"] = "ended"
        
        if target_id and target_id in active_calls:
            active_calls[target_id]["state"] = "ended"
        
        # Emit ONLY ONCE to each user
        socketio.emit("call_ended", room=str(user_id))
        print(f"📢 Emitted call_ended to room {user_id}")
        
        if target_id:
            socketio.emit("call_ended", room=str(target_id))
            print(f"📢 Emitted call_ended to room {target_id}")
        ...
        print(f"✅ Call ended once: {user_id} with {target_id}")
...
def debounced_status_update(user_id, is_online, delay=1.0):
    """Debounce status updates to prevent spamming"""
...
def handle_call_timeout(caller_id, target_id):
    """Handle call timeout if no answer"""
    try:
        print(f"⏰ Call timeout: caller={caller_id}, target={target_id}")
        with call_lock:
            if caller_id in active_calls and active_calls[caller_id].get('state') == 'calling':
                # Use centralized end_call function
                pass  # Will call end_call outside lock to avoid deadlock
        
        # Call end_call outside the lock
        end_call(caller_id)
...
def check_auth():
    ...
    return jsonify({'authenticated': current_user.is_authenticated, 'user_id': current_user.id if current_user.is_authenticated else None})
...
@login_required
def logout():
    """Secure logout with POST method only - NO AUTO RECONNECT"""
    user_id = current_user.id
    print(f"🚪 Logout started for {user_id}")
    
    try:
        # End any active calls using centralized function
        end_call(user_id)
        ...
        print(f"✅ Updated offline status for {user_id}")
        ...
        print(f"❌ Logout error during cleanup: {e}")
    
    # FINAL logout - clear Flask-Login session
    logout_user()
    session.clear()
    
    print(f"✅ Logout completed for {user_id}")
...
def handle_connect():
    ...
    if not current_user.is_authenticated:
        print(f"❌ Unauthorized socket connection blocked: {request.sid}")
        return False
    ...
    try:
        ...
        print(f"❌ Connect status error: {e}")
...
def handle_disconnect():
    ...
    if current_user.is_authenticated:
        ...
        print(f"👋 User {current_user.id} left room {user_room}")
        
        # End any active calls using centralized function
        end_call(current_user.id)
        ...
        try:
            ...
            print(f"❌ Disconnect status error: {e}")
...
def handle_call_user(data):
    if not current_user.is_authenticated:
        print(f"❌ Unauthenticated call attempt from {request.sid}")
    ...
    if not target_id or not call_type:
        print(f"❌ Missing call parameters from {current_user.id}")
    ...
    try:
        ...
        print(f"❌ Call user check error: {e}")
        emit('call_error', {'message': 'Cannot check user status'}, room=request.sid)
        return
    
    # Store call state
    with call_lock:
        active_calls[current_user.id] = {
            'with': target_id,
            'type': call_type,
            'state': 'calling'
        }
        print(f"📞 Call initiated: {current_user.id} -> {target_id} ({call_type})")
    
    # Start timeout timer (30 seconds)
    timeout_timer = Timer(30.0, handle_call_timeout, args=[current_user.id, target_id])
    timeout_timer.daemon = True
    timeout_timer.start()
    call_timeouts[current_user.id] = timeout_timer
    
    # Send incoming call to target user's private room
    call_data = {
    ...
    socketio.emit('incoming_call', call_data, room=str(target_id))
    print(f"📞 Incoming call sent to room {target_id}")

@socketio.on('answer_call')
def handle_answer_call(data):
    if not current_user.is_authenticated:
        print(f"❌ Unauthenticated answer attempt from {request.sid}")
        emit('call_error', {'message': 'Not authenticated'}, room=request.sid)
        return
    ...
    if not caller_id or not answer_sdp:
        print(f"❌ Missing answer parameters from {current_user.id}")
        emit('call_error', {'message': 'Missing parameters'}, room=request.sid)
        return
    ...
    with call_lock:
        if caller_id not in active_calls:
            print(f"❌ Call from {caller_id} no longer exists")
        ...
        if caller_id in active_calls:
            ...
            print(f"✅ Call connected: {caller_id} <-> {current_user.id}")
    ...
    # Send answer to caller's private room
    socketio.emit('call_answered', {'answer': answer_sdp}, room=str(caller_id))
    print(f"✅ Call answer sent to room {caller_id}")
...
def handle_reject_call(data):
    ...
    caller_id = data.get('caller_id')
    print(f"📞 Call rejected: {current_user.id} rejected call from {caller_id}")
    ...
    # End the call using centralized function
    end_call(caller_id)
...
def handle_ice_candidate(data):
    ...
    if target_id and candidate:
        print(f"🔄 ICE candidate exchanged: {current_user.id} -> {target_id}")
        socketio.emit('ice_candidate', {'candidate': candidate}, room=str(target_id))
...
def handle_end_call(data):
    ...
    # If target_id not provided, try to find it from active_calls
    if not target_id:
        with call_lock:
            if current_user.id in active_calls:
                ...
                for uid, call in active_calls.items():
                    if call.get('with') == current_user.id:
                        target_id = uid
                        break
    
    print(f"📞 Manual call end: {current_user.id} with {target_id}")
