...
# Monkey patch for production (eventlet)
import eventlet
...
def get_unread_counts(user_id):
    """Return dict {sender_id: count} of unread messages for given user"""
...
def get_messages_between(u1, u2, limit=20, offset=0):
    """Fetch messages between two users with limit and offset (most recent first, then reversed)"""
...
def get_reactions_for_messages(message_ids):
    """Fetch all reactions for a list of message IDs in one query"""
    if not message_ids:
        return {}
...
def users():
    ...
    # Format last_seen for each user
...
def chat(user_id):
    ...
    # 2. Fetch first 20 messages (optimized)
    messages = get_messages_between(current_user.id, user_id, limit=20, offset=0)
    
    # 3. Batch fetch reactions for all messages in one query
    ...
    # 5. Mark messages as read (another query but necessary)
...
if __name__ == '__main__':
    # Use eventlet for production (render.com)
    socketio.run(app, host='0.0.0.0', port=int(os.environ.get('PORT', 5000)), 
                 debug=False, async_mode='eventlet')
