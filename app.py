...
# ========== LIVE CHAT STORAGE ==========
live_chat_messages = []  # Store messages in memory
LIVE_CHAT_MAX_MESSAGES = 500  # Keep last 500 messages
MESSAGES_PER_PAGE = 20  # Load 20 messages at a time
...
def live_chat():
    ...
    try:
        ...
        # Get recent live chat messages (last 20)
        recent_messages = live_chat_messages[-MESSAGES_PER_PAGE:] if live_chat_messages else []
        total_messages = len(live_chat_messages)
        has_more = total_messages > MESSAGES_PER_PAGE
...
def get_live_chat_messages():
    """Get paginated live chat messages"""
    try:
        offset = int(request.args.get('offset', 0))
        
        if offset > 0:
            start_idx = max(0, len(live_chat_messages) - MESSAGES_PER_PAGE - offset)
            end_idx = max(0, len(live_chat_messages) - offset)
        else:
            start_idx = max(0, len(live_chat_messages) - MESSAGES_PER_PAGE)
            end_idx = len(live_chat_messages)
        
        messages = live_chat_messages[start_idx:end_idx] if live_chat_messages else []
        ...
        return jsonify({
            ...
            'total_messages': len(live_chat_messages),
            'has_more': len(live_chat_messages) > (end_idx),
...
def send_live_chat_message():
    ...
    try:
        ...
        live_chat_messages.append(message)
        
        if len(live_chat_messages) > LIVE_CHAT_MAX_MESSAGES:
            live_chat_messages.pop(0)
        
        socketio.emit('live_message', message, to='live_chat')
...
def upload_live_chat_audio():
    """Upload recorded audio to live chat"""
    try:
...
def delete_live_message():
    ...
    try:
        ...
        for i, msg in enumerate(live_chat_messages):
            if msg['id'] == message_id:
                if delete_for == 'everyone':
                    live_chat_messages[i]['content'] = 'This message was deleted'
                    live_chat_messages[i]['message_type'] = 'text'
                    live_chat_messages[i]['is_deleted'] = True
        ...
        for msg in live_chat_messages:
            if msg['id'] == message_id:
                ...
                for idx, r in enumerate(reactions):
                    if r['user_id'] == current_user.id and r['reaction'] == reaction:
                        existing_idx = idx
                        break
