...
active_calls = {}  # {user_id: {'with': other_user_id, 'type': 'audio'/'video', 'state': 'calling'/'connected'}}
call_timeouts = {}  # {user_id: timer}
status_update_timers = {}
active_users = {}  # {user_id: {'sid': socket_id, 'username': name}}
call_lock = Lock()
    ...
    try:
        with call_lock:
            if caller_id in active_calls and active_calls[caller_id].get('state') == 'calling':
                ...
                logger.info(f"Call timeout: {caller_id} -> {target_id}")
    ...
    try:
        ...
        with call_lock:
            if current_user.id in active_calls:
                ...
                if target_id:
                    socketio.emit('call_ended', {'message': 'User logged out'}, room=target_id)
    except Exception as e:
...
def audio_call(user_id):
    try:
        ...
        if not other_user_data.data:
            flash('User not found', 'danger')
            return redirect(url_for('users'))
        return render_template('audio.call.html', other_user=other_user_data.data[0], current_user=current_user)
    except Exception as e:
...
def video_call(user_id):
    try:
        ...
        if not other_user_data.data:
            flash('User not found', 'danger')
            return redirect(url_for('users'))
        return render_template('video.call.html', other_user=other_user_data.data[0], current_user=current_user)
    except Exception as e:
...
def handle_connect():
    ...
    try:
        ...
        logger.info(f"User {current_user.id} ({current_user.username}) connected")
    except Exception as e:
...
def handle_register_user(data):
    """Register user with their ID for targeted messaging - CRITICAL FOR CALLS"""
    if not current_user.is_authenticated:
    ...
    if user_id and user_id == current_user.id:
        ...
        active_users[user_id] = {
            'sid': request.sid,
            'username': user_name or current_user.username,
            'registered_at': get_utc_time()
        }
        
        logger.info(f"✅ User REGISTERED: {user_id} ({user_name}) in room {user_id}")
        ...
        # Check for any pending calls in session storage is handled client-side
    else:
        logger.warning(f"User registration mismatch: {user_id} vs {current_user.id}")
        emit('user_registered', {
            'status': 'error',
...
def handle_call_user(data):
    ...
    caller_name = data.get('caller_name', current_user.username)
    
    if not target_id or not call_type or not offer:
        emit('call_error', {'message': 'Missing parameters'}, room=request.sid)
        return
    
    logger.info(f"📞 CALL_USER: {current_user.id} ({caller_name}) -> {target_id} ({call_type})")
    
    # Check if user is already in a call
    with call_lock:
        if current_user.id in active_calls and active_calls[current_user.id].get('state') != 'ended':
            emit('call_error', {'message': 'You are already in a call'}, room=request.sid)
            return
        
        if target_id in active_calls and active_calls[target_id].get('state') != 'ended':
            emit('call_busy', {'message': 'User is on another call'}, room=request.sid)
            return
    
    # Check if target user is online via active_users tracking
    if target_id not in active_users:
        logger.warning(f"Target user {target_id} not in active_users")
        ...
        logger.info(f"Target user {target_id} is online (active_users tracking)")
    ...
    logger.info(f"✅ Incoming call event sent to room: {target_id}")
    
    # Also send confirmation to caller
    emit('call_initiated', {
        'target_id': target_id,
        'status': 'ringing',
        'call_type': call_type
...
def handle_answer_call(data):
    ...
    logger.info(f"📞 ANSWER_CALL: {current_user.id} answering {caller_id} ({call_type})")
    ...
    with call_lock:
        ...
        if caller_id in active_calls:
            ...
            active_calls[caller_id]['answered_at'] = get_utc_time()
        
        # Also store for answering user
        active_calls[current_user.id] = {
            'with': caller_id, 
            'type': call_type, 
            'state': 'connected',
    ...
    logger.info(f"✅ Answer sent to room: {caller_id}")
...
def handle_reject_call(data):
    ...
    logger.info(f"❌ REJECT_CALL: {current_user.id} rejecting {caller_id}")
...
def handle_ice_candidate(data):
    ...
    logger.info(f"🧊 ICE candidate from {current_user.id} to {target_id}")
...
def handle_end_call(data):
    ...
    logger.info(f"📴 END_CALL: {current_user.id} ending call with {target_id}")
    
    # If target_id not provided, find it from active_calls
    if not target_id:
        with call_lock:
            if current_user.id in active_calls:
                target_id = active_calls[current_user.id].get('with')
            elif current_user.id in [call.get('with') for call in active_calls.values()]:
...
if __name__ == '__main__':
    ...
    logger.info(f"Active users tracking initialized")
